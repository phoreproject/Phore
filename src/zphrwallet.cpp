...
void CzPHRWallet::GenerateDeterministicZPHR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPHRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
    ...
    CKey key;
    SeedToZPHR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
    coin = libzerocoin::PrivateCoin(Params().Zerocoin_Params(), denom, bnSerial, bnRandomness);
...
bool CzPHRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    ...
    //Generate the coin
    libzerocoin::PrivateCoin coin(Params().Zerocoin_Params(), dMint.GetDenomination(), false);
